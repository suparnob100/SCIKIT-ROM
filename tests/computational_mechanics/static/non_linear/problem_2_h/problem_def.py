"""
Problem 1: Piecewise 1-D elasticity (nonlinear)
"""
import numpy as np
...
class ProblemNonLinear(Problem):
    ...
    def reduced_operators(self, u, param):
        ...
        R_red : ndarray
            Reduced residual.
        """
        J_mat, R_vec = self.fom_operators(u, param), self.fom_rhs(u, param)
    ...
    def rom_solver(self, cls, param):
        ...
        if cls.cur_itr == 0:
            ...
            self.D = self.dirichlet_boundary_dofs.nodal_ix
            self.a, self.l = self.bilinear_forms()[0], self.linear_forms()[0]
            # closest_train_param = np.argmin(np.linalg.norm(cls.param_list[cls.train_mask]-param,axis=1))
            # self.u0_rom = self.U.T@(cls.fos_solutions[cls.train_mask][closest_train_param]- self.T_k) # abs(np.random.rand(self.n_sel))
            # self.u0_rom = abs(np.random.rand(self.n_sel))            
            self.u0_rom = np.ones(4) * 30.06  # cold start
            # np.sort(abs(np.random.rand(self.n_sel)))
            self.u0_rom[-1] = 0
    ...
    def hyper_rom_operators(self, u, param):
        ...
        self.residual_hyper = self.residual_hyper_red.assemble_weighted(**kw)
        self.jac_hyper = self.jac_hyper_red.assemble_weighted(**kw)
    ...
    def hyper_rom_solver(self, cls, param):
        ...
        if cls.cur_itr == 0:                                   # first snapshot → build once
            ...
            # self.u0 = np.ones(4) * 30.06  # cold start
            self.u0 = np.sort(abs(np.random.rand(self.n_sel)))
            # self.u0[-1] = 0
            ...
            closest_train_param = np.argmin(np.linalg.norm(cls.param_list[cls.train_mask]-param,axis=1))
            self.u0 = self.U.T@(cls.fos_solutions[cls.train_mask][closest_train_param]- self.T_k) # abs(np.random.rand(self.n_sel))
            # self.u0 = abs(np.random.rand(self.n_sel))
            # self.u0[-1] = 0

...
from src.skrom.fom.fem_utils import unwrap_attr
from src.skrom.rom.rom_utils import rom_data_gen, load_rom_data, reconstruct_solution
...
# ─────────────────────────────────────────────────────────────
cwd = Path.cwd()
# If running inside a folder named "problem_X", use that; else default to problem_1
if cwd.name.startswith("problem_"):
    PROBLEM = cwd.name
else:
    raise ValueError("Current directory must be named 'problem_X' where X can be a number/string.")
...
def assign_properties(prob: Problem) -> Tuple:
    ...
    hyper_rom_solver  = prob.hyper_rom_solver
...
class rom_simulation:
    ...
    def run_rom_simulation(self):
        ...
        for i, param in enumerate(self.param_list_test[:self.N_rom_snap]):
            ...
            # Time the ROM solve
            t0 = time.perf_counter()
            ms_full, T_mean = self.rom_solver(cls=self, param=param)
            sol_rom = ms_full + T_mean
    ...
    def run_hyper_rom_simulation(self, z):
        """
        ...
        for i, param in enumerate(self.param_list_test[:self.N_rom_snap]):
            ...
            t0 = time.perf_counter()
            raw = self.hyper_rom_solver(cls=self, param=param)
            sol_hyper = reconstruct_solution(raw, self.V_sel, self.mean)
            dt = time.perf_counter() - t0

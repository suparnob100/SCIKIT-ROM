...
def _unify_masterclass_module_aliases() -> None:
    ...
    for alias in ("skrom.problem_classes.masterclass", "masterclass"):
...
def _load_local_problem_module() -> None:
    ...
    module_name = f"_skrom_local_problem_def_{PROBLEM}"
    ...
    try:
        ...
        raise ImportError(
            ...
            f"@register_problem('{PROBLEM}')."
        ) from exc

    _auto_register_problem_from_module(module, PROBLEM)
...
def get_problem(name: str) -> Problem:
    ...
    if name not in PROBLEM_REGISTRY:
        _load_local_problem_module()

    try:
        return PROBLEM_REGISTRY[name]()
    except KeyError:
        raise ValueError(
            f"Unknown problem '{name}'. Available problems: {list(PROBLEM_REGISTRY.keys())}. "
            f"A local problem_def.py was checked in '{PROBLEM_DIR}'. Make sure the problem "
            f"class is decorated with @register_problem('{name}')."
        )
    ...
    def run_simulation(self) -> None:
        ...
        def _atomic_save_npz(path: Path, payload: dict):
            ...
            try:
                ...
                os.replace(tmp_name, _win_long_path(path))  # atomic on same filesystem
...
class rom_simulation:
    ...
    def run_rom_simulation(self, full_order = True, new_params = None):
        ...
        full_order : bool, optional
            If ``True``, store reconstructed full-order fields. Otherwise, store
            reduced coordinates.
        new_params : sequence, optional
        ...
        -------
        rom_error : list of float
            Relative error values in percent. Returned only when ``new_params`` is
            ``None``.
        speed_up : list of float
            Time ratios for each test case. Returned only when ``new_params`` is
            ``None``.
        rom_solutions : list of ndarray
            Reconstructed full-order solutions or reduced coordinates, depending on
            ``full_order``. Returned when ``new_params`` is provided.
        """
        self.speed_up      = []
        self.rom_error     = []
        self.rom_solutions = []
        ...
        if new_params is None:
            ...
            for i, param in enumerate(self.param_list_test[:self.N_rom_snap]):
                ...
                err = 100 * np.linalg.norm(sol_fos - sol_rom) / np.linalg.norm(sol_fos)
                ...
                if full_order:
                    ...
                    self.rom_solutions.append(sol_red_.copy())
            ...
            for i, param in enumerate(new_params):
                print(f"Snap {i+1}/{len(new_params)} params={param}")
                self.cur_itr = i
    ...
    def run_hyper_rom_simulation_ecsw(self, z, full_order = True, new_params = None):
        ...
        hyper_speed_up : list of float
            Time ratios for each test case. Returned only when ``new_params`` is
            ``None``.
        hyper_rom_solutions : list of ndarray
            Reconstructed full-order solutions or reduced coordinates, depending on
            ``full_order``. Returned when ``new_params`` is provided.
        """
        self.hyper_speed_up      = []
        ...
        if new_params is None:
            ...
            for i, param in enumerate(self.param_list_test[:self.N_rom_snap]):
                ...
                self.hyper_speed_up.append(speed)

                if full_order:
                    self.hyper_rom_solutions.append(sol_hyper.copy())
                else:
                    self.hyper_rom_solutions.append(sol_red_.copy())
